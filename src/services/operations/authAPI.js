...
export function login(email, password, navigate){
  return async (dispatch) => {
     ...
     try {
      ...
      const userPayload = { ...response.data.existingUser, image: userImage }
      dispatch(setUser(userPayload))
      localStorage.setItem("token", JSON.stringify(response.data.token))
      localStorage.setItem("user", JSON.stringify(userPayload))
      let redirectPath = "/dashboard/my-profile"
      if (
        userPayload.accountType === ACCOUNT_TYPE.INSTRUCTOR &&
        !userPayload.approved
      ) {
        redirectPath = "/pending-approval"
      } else if (userPayload.accountType === ACCOUNT_TYPE.ADMIN) {
